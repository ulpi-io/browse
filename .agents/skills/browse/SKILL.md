...
description: |
  ...
  Persistent headless Chromium daemon for web. Android, iOS, and macOS app automation via
  accessibility APIs. Browse URLs, read content, click elements, fill forms, run JavaScript,
  take screenshots, automate native apps. ~100ms per command. Auto-installs Android toolchain.
  Works with Claude Code, Cursor, Cline, Windsurf, and any agent that can run Bash.
allowed-tools:
  - Bash
  - Read
